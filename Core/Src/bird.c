...
static void pipes_draw(void) {
    for (int i = 0; i < NUM_PIPES; i++) {
        lcd_fill(pipes[i].x, 0, pipes[i].x + PIPE_WIDTH,
                 pipes[i].gap_y - PIPE_GAP / 2, PIPE_COLOR);
        lcd_fill(pipes[i].x, pipes[i].gap_y + PIPE_GAP / 2,
                 pipes[i].x + PIPE_WIDTH, 219, PIPE_COLOR);
    }
}
